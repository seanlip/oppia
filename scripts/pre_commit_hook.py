...
from typing import Final, List, Optional, Tuple  # pylint: disable=wrong-import-position
...
def check_changes_in_config() -> None:
    ...
    if not check_changes('feconf'):
        raise Exception(
            'Changes to %s made for deployment cannot be committed.' % (FECONF_FILEPATH)
        )
...
def main(args: Optional[List[str]] = None) -> None:
    ...
    if does_diff_include_package_lock_file() and (
        ...
        sys.exit(1)
    print('Running prettier and black ...')
    run_formatters()
