...
from pylint import (  # isort:skip  pylint: disable=wrong-import-order, wrong-import-position
    checkers,
)
from pylint import (  # isort:skip  pylint: disable=wrong-import-order, wrong-import-position
    interfaces,
)
from pylint.checkers import (  # isort:skip  pylint: disable=wrong-import-order, wrong-import-position
    utils as checker_utils,
)
from pylint.extensions import (  # isort:skip  pylint: disable=wrong-import-order, wrong-import-position
    _check_docs_utils,
)
            ...
            if token_type == tokenize.OP and token in ('(', ')'):
                ...
                if excluded:
                    ...
                    if '#' in split_line:
                        ...
                        if split_line[comment_index - 1].endswith(':') or split_line[
                            comment_index - 1
                        ].endswith('):'):
                            excluded = False
                ...
                for char_num in range(line_length):
                    char = line[char_num]
                    if in_string and (
                        char == escape_character_indicator or escape_character_found
                    ):
...
class DocstringParameterChecker(checkers.BaseChecker):  # type: ignore[misc]
    ...
    * Check that all explicitly raised exceptions in a function are documented
    ...
    def check_typeinfo(
        self, node: astroid.nodes.FunctionDef, node_doc: _check_docs_utils.Docstring
    ) -> None:
        ...
        if node.doc:
            ...
            for line in docstring:
                ...
                # Check if we are in a docstring raises section.
                    ...
                    and current_docstring_section == self.DOCSTRING_SECTION_RAISES
                ):
                ...
                # section.
                    ...
                    # indentation the same as the definition line.
                        ...
                        if (
                            current_line_indentation != args_indentation_in_spaces + 4
                ...
                # indentation.
                    ...
                    if current_line_indentation % 4 != 0:
                        self.add_message(
                            'incorrect-indentation-for-arg-header-doc', node=node
                        )
    ...
    def check_functiondef_returns(
        ...
        if (node_doc.has_returns() or node_doc.has_rtype()) and not any(
            docstrings_checker.returns_something(ret_node) for ret_node in return_nodes
        ):
    ...
    def check_arguments_in_docstring(
        ...
        * If there's no Sphinx style, Google style or NumPy style parameter
        ...
        def _compare_missing_args(
            found_argument_names: Set[str], message_id: str, not_needed_names: Set[str]
        ) -> None:
        ...
        for module_regex, forbidden_imports in module_to_forbidden_imports:
            ...
            for forbidden_import in forbidden_imports.split('|'):
                ...
                if stripped_forbidden_import.startswith('from'):
                    from_part, import_part = stripped_forbidden_import[4:].split(
                        ' import '
                    )
...
class SingleLineCommentChecker(checkers.BaseChecker):  # type: ignore[misc]
    ...
    def _check_comment_starts_with_capital_letter(
        ...
        allowed_prefix_is_present = any(
            line[2:].startswith(word) for word in self.config.allowed_comment_prefixes
        )
    ...
    def _check_punctuation(self, line: str, line_num: int) -> None:
        ...
        if last_char_is_invalid and not (
            excluded_phrase_is_present_at_end or excluded_phrase_at_beginning_of_line
        ):
    ...
    def _check_trailing_comment_starts_with_allowed_pragma(
        ...
        allowed_inline_pragma_present = any(
            line[2:].startswith(word) for word in ALLOWED_PRAGMAS_FOR_INLINE_COMMENTS
        )
        ...
        for token_type, token, (line_num, _), _, line in tokens:
            if token_type == tokenize.NAME and token in self.keywords:
                ...
                if not re.search(r'(\s|^)' + token + r'(\s[^\s]|$)', line):
                    self.add_message(
                        'single-space-after-keyword', args=(token), line=line_num
                    )
    ...
    def _check_exceptional_type_is_documented(
        ...
        if type_status_dict['outside_function_signature_block']:
            if type_status_dict['type_present_in_function_signature']:
                if type_status_dict['type_comment_pending'] and type_status_dict[
                    'func_def_start_line'
                ] <= (
                    ...
                    self._add_exceptional_type_error_message(
                        exceptional_type, type_status_dict['func_def_start_line'], node
                    )
