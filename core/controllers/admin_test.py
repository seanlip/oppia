...
class AutomaticVoiceoverAdminControlHandlerTests(test_utils.GenericTestBase):
    """Tests for Azure admin config for automatic voiceovers."""
    ...
    def test_handler_should_be_able_to_return_azure_config(self) -> None:
        response = self.get_json(
            ...
            expected_status_int=200)
        self.assertTrue(
            response['voiceover_autogeneration_using_azure_is_enabled'])

    def test_admin_should_be_able_to_update_azure_config(self) -> None:
        self.login(self.CURRICULUM_ADMIN_EMAIL, is_super_admin=True)
        csrf_token = self.get_new_csrf_token()
        ...
        voiceover_services.update_azure_config_for_voiceover_autogeneration(
            False)
        self.assertFalse(
            voiceover_services.is_voiceover_autogeneration_using_azure_enabled()
        )
    ...
    def test_non_admin_should_not_be_able_to_update_azure_config(self) -> None:
        csrf_token = self.get_new_csrf_token()
